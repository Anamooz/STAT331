...
suspects <- suspects |> 
  ...
  mutate(Time.Spotted = force_tz(time = Time.Spotted, tzone = "America/Los_Angeles")) 
```
...
suspects <- suspects |>
  filter(wday(Time.Spotted) != 3) |>
  filter(wday(Time.Spotted) != 5)
```
...
suspects <- suspects |>
  filter(Time.Spotted %within% (ymd("2022-10-20", tz = "America/Los_Angeles") %--% ymd("2022-12-29", tz = "America/Los_Angeles")))
```
...
suspects <- suspects |>
  mutate(Time.Iceland = with_tz(time = Time.Spotted, tzone = "Atlantic/Reykjavik")) |>
  filter(hour(Time.Iceland) >= 16 & hour(Time.Iceland) < 24)
```
...
suspects <- suspects |>
  filter(day(Time.Spotted) >= 22 & day(Time.Spotted) < 26)
```
