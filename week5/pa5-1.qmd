...
```{r}
str_subset(message, pattern = "[:punct:]$")
```
...
```{r}
str_extract_all(message, pattern = "[:punct:]", simplify = TRUE) |>
  str_flatten()
```
...
str_trim(message, side = "both") |>
  ...
  str_remove_all(pattern = "ugh*[:punct:]") |>
  str_replace_all(pattern = "a{2}", replacement = "ee") |>
  str_replace_all(pattern = "z", replacement = "t") |>
  str_replace_all(pattern = "b$", replacement = "y") |>
  str_replace_all(pattern = "^[Kk]", replacement = "v") |>
