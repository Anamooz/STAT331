...
```{r}
my_symbolsNum <- 1:len
my_symbols <- data.frame(my_symbols, my_symbolsNum)
```
...
len <- length(secret_vector)
len
count1 = 0
count2 = 0
for (i in 500:len){
  if (secret_vector[i] < 100){
    count1 = count1 + 1
  } else {
    count2 = count2 + 1
  }
...
```{r}
secret_vector[38:465] <- sqrt(secret_vector[38:465])
```

11. Use the `round()` function to round all numbers to the nearest whole number.

```{r}
secret_vector <- round(secret_vector)
```
...
```{r}
secret_vector[secret_vector == 39] <- 20
secret_vector
```
...
# Should be 344!
temp <- secret_vector[secret_vector %% 2 == 0]
length(temp)
```
...
```{r}
stringr::str_c(my_symbols$Symbol[secret_vector], collapse = "")
```
