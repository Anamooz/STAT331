...
pluralize_gift <- function(gift){
  ...
  gift <- str_replace(gift, "y$", "ie")
  # Replace 'oo' with 'ee'
  gift <- str_replace(gift, "oo", "ee")
  
  # Only add 's' if it doesn't already end in 'eese' or 'en' (like children/oxen)
  if (!str_detect(gift, "(eese|en)$")) {
    gift <- str_c(gift, "s")
  }
...
make_phrase <- function(num, item, verb, adjective, location) {
  ...
  starts_with_vowel <- str_detect(item, "^[aeiou]")
  ...
  ### See the tip above about turning numbers into words (e.g. 10 into ten).
    # Step 4: Build day_word
  number_word <- as.character(english(num))
  day_word <- if (num == 1) {
...
```{r}
xmas2 <- xmas |> 
  mutate(full_phrase = pmap_chr(.l = list(num       = Day,
                                          item      = Gift.Item, 
                                          verb      = Verb, 
                                          adjective = Adjective, 
                                          location  = Location), 
