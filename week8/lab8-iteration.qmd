...
# Data from Lab 2
surveys <- read_csv("../week2/surveys.csv")

# Data from Lab 3
evals <- read_csv("../week3/teacher_evals.csv") |> 
  rename(sex = gender)

# Data from Lab 7
fish <- read_csv("../week7/BlackfootFish.csv")
```
...
# Q1 code, https://chatgpt.com/share/682f66b0-d1ac-8009-af84-86697f9860d2
...
surveys_table |>
  arrange(type) |>
  kable(col.names = c("Var_Name", "Type"),
        caption = "The Data Types and Names of each Column from the Surveys Data",
        format = "html") |>
...
# Q4 code, https://chatgpt.com/share/682f97db-28f4-8009-9e06-dd438ab9ae79
evals_table <- evals |>
  ...
  select(sex, sen_level, academic_degree) |>
  map_at(c("sex", "sen_level", "academic_degree"), ~ count(tibble(var = .x), var)) |>
  list_rbind(names_to = "variable") |>
  rename(level = var, count = n) |>
  group_by(variable) |>
...
evals_table |> 
  ...
  kable(digits = c(0, 0, 0, 2),
        caption = "Demographic of Professors",
...
# Q6 code, https://chatgpt.com/share/6830dedc-bf64-8009-b1f7-eaa9ecd2f4fd

file_reader <- function(directory) {
  names <- list.files(directory, pattern = "\\.csv$")
  full_paths <- file.path(directory, names)
  files_read <- map(full_paths, ~ read_csv(.x))
  names(files_read) <- file_path_sans_ext(names)
  return(files_read)
}

names(file_reader("../week7"))
is.list(file_reader("../week7"))
```
...
fish |>
  map_int(~ sum(is.na(.x))) |>
  enframe(name = "variable", value = "x") |>
  arrange(desc(x)) |>
  kable(caption = "Missing Values for Every Variable in Fish Data",
        col.names = c("Variable", "No. Missing")) |>
  kable_classic(full_width = F, bootstrap_options = "striped") |>
  row_spec(row = 0, bold = T, align = "c") |>
  kable_styling(full_width = F, bootstrap_options = "striped")
